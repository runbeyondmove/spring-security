...
1. 编写验证码过滤器
2. 放在UsernamePasswordAuthenticationFilter过滤器之前: Spring
   Security实际上是由许多过滤器组成的过滤器链，处理用户登录逻辑的过滤器为UsernamePasswordAuthenticationFilter，
   而验证码校验过程应该是在这个过滤器之前的，即只有验证码校验通过后才去校验用户名和密码。
